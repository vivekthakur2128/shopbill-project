...
// .then(data => {
    ...
    generateBill(indexOfSelectedMonth, unitRate, shopDetails, date, noOfDays, shopId);
  // })
  }
  ...
  function getShopId() {
    let select = document.getElementById("selectShopId");
    let option = select.options[select.selectedIndex];
    shopIdToGenerateBill = option.text;
    return shopIdToGenerateBill;
  }
  ...
  function generateBill(indexOfSelectedMonth, unitRate, shopDetails, date, noOfDays, shopId){
...
//   for (let index = 0; index < shopDetails.length; index++) { 
    if ("content" in document.createElement("template")) { 
        ...
        div[0].textContent = `${shopDetails[shopId-1][1]}`; 
        ...
        div[2].textContent = `${shopDetails[shopId-1][indexOfSelectedMonth]} - ${shopDetails[shopId-1][indexOfSelectedMonth - 1]} = ${ 
          shopDetails[shopId-1][indexOfSelectedMonth] - 
          shopDetails[shopId-1][indexOfSelectedMonth - 1] 
        }`;
        div[3].textContent = `${shopDetails[shopId-1][indexOfSelectedMonth] - shopDetails[shopId-1][indexOfSelectedMonth - 1]} * ${unitRate} = ${(shopDetails[shopId-1][indexOfSelectedMonth] - shopDetails[shopId-1][indexOfSelectedMonth - 1]) * unitRate}`; 
